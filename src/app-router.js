...
(function(window, document) {
  ...
  AppRouter.init = function() {
    ...
    // <app-router core-animated-pages transitions="hero-transition cross-fade"> or
    // <app-router animated-pages="neon-animated-pages" animation-initial-selection>
    if (router.hasAttribute('core-animated-pages') || router.hasAttribute('animated-pages')) {
      // use shadow DOM to wrap the <app-route> elements in a <*-animated-pages> element
      // <app-router>
      //   # shadowRoot
      //   <*-animated-pages>
      ...
      if (router.hasAttribute('core-animated-pages')) {
        router.setAttribute('animated-pages', 'core-animated-pages');
      }
      router.animatedPages = document.createElement(router.getAttribute('animated-pages'));
      router.animatedPages.appendChild(document.createElement('content'));

      // don't know why it needs to be static, but absolute doesn't display the page
      router.animatedPages.style.position = 'static';

      // toggle the selected page using selected="path" instead of selected="integer"
      // for <core-animated-pages>
      router.animatedPages.setAttribute('valueattr', 'path');
      // for <neon-animated-pages>
      router.animatedPages.setAttribute('attr-for-selected', 'path');
      ...
      // for <core-animated-pages>
      router.animatedPages.setAttribute('transitions', router.getAttribute('transitions'));
      // for <neon-animated-pages>
      router.animatedPages.setAttribute('animate-initial-selection', router.getAttribute('animate-initial-selection') || 'false');

      // set the shadow DOM's content
      router.shadowRoot.appendChild(router.animatedPages);
      ...
      // for <core-animated-pages>
      router.animatedPages.addEventListener('core-animated-pages-transition-end', onTransitionEnd);
      // for <neon-animated-pages>
      router.animatedPages.addEventListener('neon-animation-finish', onTransitionEnd);
    }
  ...
  function activateElement(router, element, url, eventDetail) {
    // when using *-animated-pages, the router doesn't remove the previousRoute's content right away. if you
    ...
    // remove the old route's content before loading the new route. *-animated-pages temporarily needs the old and
    // new route in the DOM at the same time to animate the transition, otherwise we can remove the old route's content
    // right away. there is one exception for *-animated-pages where the route we're navigating to matches the same
    ...
    if (!router.hasAttribute('animated-pages') || eventDetail.route === eventDetail.oldRoute) {
      deactivateRoute(router.previousRoute);
    }
    ...
    // animate the transition if *-animated-pages are being used
    if (router.hasAttribute('animated-pages')) {
      router.animatedPages.selected = router.activeRoute.getAttribute('path');
      // the 'core-animated-pages-transition-end' or 'neon-animation-finish' event handler in init() will call deactivateRoute() on the previousRoute
    }

    // scroll to the URL hash if it's present
    if (url.hash && !router.hasAttribute('animated-pages')) {
      scrollToHash(url.hash);
    }
